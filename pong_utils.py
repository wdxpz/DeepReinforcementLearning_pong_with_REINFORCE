...
# collect trajectories for a parallelized parallelEnv object, but return probs in tensor
def collect_trajectories2(envs, policy, tmax=200, nrand=5):
    ...
    for t in range(tmax):
        ...
        # no gradient propagation is needed
        # so we move it to the cpu
        probs = policy(batch_input).squeeze()
        probs_np = probs.squeeze().detach().cpu().numpy()
        
        
        action = np.where(np.random.rand(n) < probs_np, RIGHT, LEFT)
        probs = torch.where(torch.tensor(action, dtype=torch.int8, device=device)==torch.tensor(np.ones((8))*RIGHT, dtype=torch.int8, device=device), probs, 1.0-probs)
        probs_np = np.where(action==RIGHT, probs_np, 1.0-probs_np)
    ...
    # return pi_theta, states, actions, rewards, probability
    return torch.stack(prob_list, dim=0), state_list, \
        action_list, reward_list
